...
export function KpiCard({ label, value, hint, delta, icon: Icon, variant = 'neutral', index = 0 }: Props) {
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {hint && (
            <div className={cn('mt-1 text-xs', onColor ? 'text-white/70' : 'text-muted-foreground')}>
              {hint}
            </div>
          )}
        </div>
