...
import { ArrowLeft, CalendarDays, FileSpreadsheet, Layers, Percent, PiggyBank, Trash2 } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          ...
          index={0}
        />
        <KpiCard label="Monthly EMI" value={currency(summary.emi)} hint="Reducing balance" icon={Layers} variant="success" index={1} />
        <KpiCard label="Total interest" value={currency(summary.totalInterest)} hint="Lifetime cost" icon={Percent} variant="warm" index={2} />
        <KpiCard label="Payoff date" value={format(parseISO(summary.payoffDate), 'MMM yyyy')} hint={`${summary.payoffMonths} months`} icon={CalendarDays} variant="cool" index={3} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="glass-card rounded-2xl p-5">
          ...
          <PrincipalInterestPie principal={Number(loan.borrowed_amount)} interest={summary.totalInterest} />
        </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-card grid grid-cols-2 gap-4 rounded-2xl p-6 md:grid-cols-4">
        ...
        <Meta label="Balloon" value={loan.balloon_amount ? `${currency(Number(loan.balloon_amount))} · ${loan.balloon_date}` : '—'} />
      </motion.div>
    </div>
  );
}
