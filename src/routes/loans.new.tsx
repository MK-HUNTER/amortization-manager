...
export const Route = createFileRoute('/loans/new')({
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'Add a loan with strict validation and a live amortization preview.' },
    ],
  }),
  ...
  const onSubmit = handleSubmit(async (values) => {
    try {
      const payload = {
        ...
        balloon_amount: values.balloon_amount && !Number.isNaN(Number(values.balloon_amount)) ? Number(values.balloon_amount) : null,
        extra_payment: values.extra_payment && !Number.isNaN(Number(values.extra_payment)) ? Number(values.extra_payment) : 0,
          ...
          <Section title="Lender">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Bank / Lender" error={errors.bank_name?.message}>
                <input className={inputCls(!!errors.bank_name)} placeholder="ICICI Bank" {...register('bank_name')} />
              </Field>
              <Field label="Loan number" error={errors.loan_number?.message}>
                <input className={inputCls(!!errors.loan_number)} placeholder="HL-2024-00138" {...register('loan_number')} />
              </Field>
              <Field label="Purpose" error={errors.purpose?.message} className="sm:col-span-2">
                <input className={inputCls(!!errors.purpose)} placeholder="Home renovation" {...register('purpose')} />
              </Field>
            </div>
          </Section>
          ...
          <Section title="Loan terms">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <Field label="Borrowed amount" error={errors.borrowed_amount?.message}>
                <input type="number" step="0.01" min="0" className={inputCls(!!errors.borrowed_amount)} placeholder="250000" {...register('borrowed_amount')} />
              </Field>
              <Field label="Interest rate (%)" error={errors.interest_rate?.message}>
                <input type="number" step="0.001" min="0" max="100" className={inputCls(!!errors.interest_rate)} placeholder="8.5" {...register('interest_rate')} />
              </Field>
              <Field label="Tenure (months)" error={errors.tenure_months?.message}>
                <input type="number" min="1" max="600" className={inputCls(!!errors.tenure_months)} placeholder="240" {...register('tenure_months')} />
              </Field>
              <Field label="Start date" error={errors.start_date?.message}>
                <input type="date" className={inputCls(!!errors.start_date)} {...register('start_date')} />
              </Field>
          ...
          <Section title="Optional">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <Field label="Extra monthly payment">
                <input type="number" step="0.01" min="0" className={inputCls(false)} placeholder="0" {...register('extra_payment')} />
              </Field>
              <Field label="Balloon date">
                <input type="date" className={inputCls(false)} {...register('balloon_date')} />
              </Field>
              <Field label="Balloon amount">
                <input type="number" step="0.01" min="0" className={inputCls(false)} placeholder="0" {...register('balloon_amount')} />
              </Field>
            </div>
          </Section>
